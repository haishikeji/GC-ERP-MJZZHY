...
import cn.px.module.finance.v2.dao.StampInvoiceReceivableDao;
import cn.px.module.finance.v2.model.*;
import cn.px.module.finance.v2.service.PaymentStampReceivableInvoiceDetailService;
...
@RequestMapping(value = "/module/stampInvoiceReceivable")
public class StampInvoiceReceivableController {

    @Resource
    StampInvoiceReceivableDao stampInvoiceReceivableDao;
    ...
    public Result getListLeft(@RequestBody Map<String, Object> conditions) {
        ...
        //分页数据
        map.put("list", stampInvoiceReceivableDao.getListLeft(conditions));
        if (start != null) {
            //总页数
    ...
    public Result getListDown(@RequestBody Map<String, Object> conditions) {
        ...
        conditions.put("start", start);
        //分页数据
        List<StampDownModel> lst = stampInvoiceReceivableDao.getListDown(conditions);
        map.put("list", lst);
        if (start != null) {
            //总页数
            // map.put("total", invoiceService.getTotal(conditions));
    ...
    @Resource
    PaymentStampReceivableInvoiceDetailService paymentStampReceivableInvoiceDetailService;

    @ApiOperation(value = "保存发票明细", notes = "保存发票明细")
    @PostMapping("/detailSave")
    @Transactional(rollbackFor = Exception.class)
    @ResponseBody
    public Result detailSave(@RequestBody PaymentStampReceivableInvoiceDetail arg) {
        Result result = null;
        try {
            paymentStampReceivableInvoiceDetailService.saveOrUpdate(arg);
    ...
    @ResponseBody
    @Transactional(rollbackFor = Exception.class)
    public Result detailDel(@RequestBody List<PaymentStampReceivableInvoiceDetail> list) {
        Result result = null;
        try {
            if (list.size() != 1 ) {
                throw new RuntimeException("禁止批量删除");
            }
            paymentStampReceivableInvoiceDetailService.removeBatchByIds(list);
    ...
    public Result exportDownList(@RequestBody Map<String, Object> conditions) throws Exception {
        ...
        try {
            ...
            List<StampLeft1Model> heads = null;
            StampLeft1Model head = null;
            List<StampDownModel> details = null;
            ExportDownModel sum = null;


            heads = stampInvoiceReceivableDao.getListLeft(conditions);
            head = heads.get(0);
            details = stampInvoiceReceivableDao.getListDown(conditions);
            ...
            if (details != null && details.size() >= 1) {
                BigDecimal sumBorrowAmount = details.stream().map(StampDownModel::getBorrowAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal sumStayAmount = details.stream().map(StampDownModel::getStayAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
            ...
            JxlsBuilder jxlsBuilder = JxlsBuilder
                    .getBuilder("excels/stampContractInvoice.xlsx")
                    .out(outPath)
                    .putVar("head", head)
